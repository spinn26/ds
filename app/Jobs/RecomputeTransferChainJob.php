...
class RecomputeTransferChainJob implements ShouldQueue
{
     ...
     *
     * ⚠ Два умышленных отличия от прочих обходов дерева, оба проверены
     * тестом TreeTraversalSafetyTest:
     *   - UNION, а не UNION ALL: он схлопывает повторы, поэтому цикл в
     *     структуре обход не подвешивает и отдельный лимит глубины здесь не
     *     нужен (с ним, наоборот, пришлось бы тащить в ключ дедупа глубину, и
     *     цикл снова стал бы бесконечным);
     *   - фильтра по dateDeleted НЕТ намеренно: мягко удалённый партнёр в
     *     середине ветки не должен отрезать своих живых потомков от
     *     пересчёта. На проде такой узел есть.
     *
     * @return array<int>
     */
