...
class ApplyPoolJob implements ShouldQueue
{
    ...
    public function handle(PoolRunner $runner, PeriodFreezeService $freeze): void
    {
        ...
        try {
            ...
            // ⛔ Период БОЛЬШЕ НЕ ЗАКРЫВАЕТСЯ автоматически (2026-08-17).
            // Раньше фиксация пула сразу звала $freeze->close(...) с пометкой
            // «Зафиксировано через расчёт пула». Владелец об этом не знал:
            // июль оказался закрыт до окончания сверки, а закрытие в довесок
            // раскрывало партнёрам отчёты, которые специально скрыли.
            // Закрытие месяца — отдельное осознанное действие на /admin/periods
            // ПОСЛЕ сверки. Пул при этом можно пересчитать и зафиксировать
            // повторно (poolLog пишется DELETE-then-INSERT, идемпотентно), а
            // уже закрытый период по-прежнему защищён проверкой isFrozen выше.
            ...
            NotificationController::notifyRoles(
                ...
                sprintf('Записано %d строк, выплачено %s ₽. Период НЕ закрыт — закройте его на странице «Периоды» после сверки.',
                    $result['written'] ?? 0,
            ...
            $this->setProgress('done', 100, sprintf(
                'Готово: %d строк, %s ₽. Период остаётся открытым — закрыть после сверки.',
                $result['written'] ?? 0,
                number_format($result['totalPaid'] ?? 0, 0, '.', ' ')
            ), [
