...
class PartnerStatusService
{
    ...
    private function logStatusChange(
        ...
        if ($from !== null && $consultant->webUser) {
            // Передаём source, а НЕ $comment: комментарий — запись аудита с
            // трассировкой IP/UA, партнёру он не уходит (см. notifyStatusChange).
            $this->notifyStatusChange($consultant->webUser, $to, $source);
        }
    }

    /**
     * Уведомление ПАРТНЁРУ о смене его статуса (кабинет + зеркало в Telegram).
     *
     * ⚠ Сюда НЕЛЬЗЯ подставлять $comment из logStatusChange: это запись аудита
     * для админа, и она содержит служебное («Самовосстановление 1/3 (терминаций:
     * 3)», «Терминация #3») плюс трассировку `IP …, UA …`. Раньше комментарий
     * уходил партнёру дословно — человек получал в Telegram свой IP и строку
     * браузера, что читается как предупреждение о взломе. Подробности остаются
     * в журнале статусов и activity_log; партнёру — человеческий текст.
     */
    private function notifyStatusChange(int $userId, PartnerActivity $to, string $source): void
    {
        ...
        [$title, $message] = match ($to) {
            ...
            PartnerActivity::Terminated => [
                'Статус: Терминация',
                "Агентский договор расторгнут: условия активационного периода не выполнены "
                    . "(нужно {$points} ЛП за {$days} дней). Восстановить участие можно "
                    . 'в личном кабинете — окно возврата откроется при входе.',
            ],
            PartnerActivity::Excluded => [
                'Статус: Исключён',
                "Все {$limit} попытки восстановления использованы, аккаунт переведён в статус "
                    . '«Исключён». Вернуть участие через личный кабинет больше нельзя — '
                    . 'если считаете, что произошла ошибка, напишите в поддержку.',
            ],
            // ⚠ В «Зарегистрирован» ведут два разных события, и текст у них
            // противоположный: самовозврат партнёра (портфель ОСТАЛСЯ у
            // наставника) и отмена админом ошибочной терминации (портфель
            // ВЕРНУЛСЯ). Различаем по source, иначе половине адресатов уходит
            // неправда.
            PartnerActivity::Registered => $source === 'self'
                ? [
                    'Статус: Зарегистрирован',
                    "Участие восстановлено. На активацию снова {$days} дней и {$points} ЛП. "
                        . 'Баллы обнулены, клиенты и контракты остались у наставника.',
                ]
                : [
                    'Статус: Зарегистрирован',
                    'Статус партнёрского аккаунта возвращён к «Зарегистрирован». '
                        . 'Подробности — в личном кабинете.',
                ],
            PartnerActivity::Inactive => [
                'Статус обновлён',
                'Статус партнёрского аккаунта изменён. Подробности — в личном кабинете.',
            ],
        };
