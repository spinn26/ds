...
class DsCommissionSync
{
    ...
    public static function syncFromTariffs(int $legacyProgramId, array $tariffs, bool $apply, bool $fillGaps = false, bool $prune = false): array
    {
        ...
        // ⚠⚠ PRUNE РАБОТАЕТ ТОЛЬКО КАК ОТЧЁТ — записи НЕ делает.
        //
        // Причина: матчер выше признаёт совпадение лишь при РОВНО одном
        // кандидате (term + свойство/год). Всё остальное — 0 совпадений при
        // запрете создания, >1 кандидата, пропуски по алиасам — уходит в
        // $skips и НЕ попадает в $matchedIds. Такая строка неотличима от
        // сироты, хотя она законная.
        //
        // Прогон 25.08.2026 показал 592 «сироты» при 9 настоящих: под нож шла
        // вся матрица по годам КВ у FFP и единственный живой тариф Axevil (3%).
        // Прежде чем разрешать запись, матчер надо сделать точным — иначе синк
        // сотрёт действующие ставки.
        if ($prune && $usableTariffs > 0) {
            ...
            // Намеренно НЕ пишем даже при $apply — см. предупреждение выше.
        }
