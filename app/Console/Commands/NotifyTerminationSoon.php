...
class NotifyTerminationSoon extends Command
{
    ...
    public function handle(): int
    {
        ...
        $withMail = $rows->filter(fn ($r) => filter_var($r->email ?? '', FILTER_VALIDATE_EMAIL))->count();
        $this->info("Отправлено уведомлений: {$sent} (в кабинет всем).");
        $this->info("Telegram: {$withTg}, письма: {$withMail} — рассылает NotificationController::create()"
            . ' через очередь, см. SendNotificationEmail.');
