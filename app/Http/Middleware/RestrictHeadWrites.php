...
class RestrictHeadWrites
{
    ...
    /**
     * Секции, где право на запись решает матрица «Группы и права», а не этот
     * гард. Общий read-only для head сохраняется: сюда попадает только то,
     * что осознанно открыли руководителю.
     *
     ...
     *
     * ⚠ Почему список явный, а не «всё, где в матрице стоит full».
     * Часть уровней в матрице проставлена миграциями ради ВИДИМОСТИ пункта
     * меню, а не ради права писать: 2026_06_24_000030 выдала всем staff-ролям
     * full на `tasks` и `org-structure` только чтобы сохранить прежнее «видно
     * всем». Отдай мы решение матрице целиком — руководитель молча получил бы
     * право править оргструктуру компании. Поэтому каждая секция добавляется
     * сюда осознанно.
     *
     * reports (22.09.2026): в матрице у head стоит «Полный», кнопка
     * «Сгенерировать отчёт» в UI показывалась, а сервер отвечал 403 — матрица
     * обещала одно, платформа делала другое.
     */
    private const SECTION_EXCEPTIONS = ['news', 'reports'];
