...
class AdminDuplicatesController extends Controller
{
    ...
    /**
     * Карточки клиентов, привязанные к контакту (person) другого человека, и
     * кандидаты на правильную привязку — person с тем же ФИО.
     */
    public function clients(Request $request): JsonResponse
    {
        ...
        $rows = DB::table('client as cl')
            ->join('person as p', 'p.id', '=', 'cl.person')
            ->whereNull('cl.dateDeleted')
            ->whereNotNull('cl.personName')
            ->whereRaw('btrim(lower(p."lastName" || \' \' || p."firstName" || \' \' || coalesce(p.patronymic,\'\')))
                     <> btrim(lower(cl."personName"))')
            ->when($search !== '', fn ($q) => $q->where('cl.personName', 'ilike', "%{$search}%"))
            ->orderBy('cl.personName')
            ->limit(500)
            ->select([
                'cl.id', 'cl.personName', 'cl.person', 'cl.email', 'cl.phone', 'cl.consultantName',
                'p.lastName as pLast', 'p.firstName as pFirst', 'p.patronymic as pPatr',
                'p.email as pEmail', 'p.phone as pPhone',
            ])
            ->get();

        // Кандидаты: person с ФИО карточки. Одним запросом на всю выдачу.
        $names = $rows->pluck('personName')->map(fn ($n) => mb_strtolower(trim((string) $n)))->unique()->values();
        $candidates = collect();
        if ($names->isNotEmpty()) {
            $candidates = DB::table('person')
                ->whereRaw('btrim(lower("lastName" || \' \' || "firstName" || \' \' || coalesce(patronymic,\'\'))) = any(?)', [
                    '{' . $names->map(fn ($n) => '"' . str_replace('"', '\"', $n) . '"')->implode(',') . '}',
                ])
                ->get(['id', 'lastName', 'firstName', 'patronymic', 'email', 'phone'])
                ->groupBy(fn ($p) => mb_strtolower(trim(
                    ($p->lastName ?? '') . ' ' . ($p->firstName ?? '') . ' ' . ($p->patronymic ?? '')
                )));
        }

        $data = $rows->map(function ($r) use ($candidates) {
            $key = mb_strtolower(trim((string) $r->personName));
            ...
            return [
                ...
                'phone' => $r->phone,
                'personId' => $r->person,
                'personName2' => trim(($r->pLast ?? '') . ' ' . ($r->pFirst ?? '') . ' ' . ($r->pPatr ?? '')),
                'personEmail' => $r->pEmail,
                'personPhone' => $r->pPhone,
                'candidates' => ($candidates[$key] ?? collect())->map(fn ($p) => [
                    'id' => $p->id,
    ...
    /**
     * Перепривязать карточку к правильному контакту или отвязать совсем.
     * Контакты самой карточки не трогаем — она ими владеет.
     */
    public function relinkClient(Request $request): JsonResponse
    {
        $data = $request->validate([
            'clientId' => ['required', 'integer', 'exists:client,id'],
            'personId' => ['nullable', 'integer', 'exists:person,id'],
        ]);

        DB::table('client')->where('id', $data['clientId'])->update([
            'person' => $data['personId'] ?? null,
            'dateChanged' => now(),
        ]);
        Audit::log('relink_person', 'client', (int) $data['clientId'], ['person' => $data['personId'] ?? null]);

        return response()->json([
            'message' => $data['personId']
                ? 'Карточка перепривязана к выбранному контакту.'
                : 'Карточка отвязана от контакта.',
        ]);
    }
}
