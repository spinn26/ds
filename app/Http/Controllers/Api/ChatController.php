...
class ChatController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        // Закрытые/решённые НЕ скрываем по умолчанию — иначе при правиле
        // «вижу только свои тикеты» пользователь часто видит пустоту,
        // потому что большинство его тикетов уже resolved. В UI остаётся
        // фильтр-чипы «Решён» / «Закрыт» для фокусной выборки.
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if ($recipientId) {
            ...
            $dup = DB::table('chat_tickets')
                ...
                ->value('id');
            if ($dup) {
                return response()->json(['ticket' => ['id' => $dup], 'deduplicated' => true]);
            }
        }
    ...
    public function sendMessage(Request $request, int $id): JsonResponse
    {
        ...
        $request->validate([
            ...
            // Гейт по расширению имени + по содержимому. ВАЖНО: docx/xlsx —
            // это zip-контейнеры, и libmagic сервера отдаёт их как
