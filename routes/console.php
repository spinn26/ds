...
// Сторож расхождений снимка начислений — с автопочинкой.
//
...
//
// Сторож нашёл расхождение — сам пересобирает снимок (resyncMonth) и шлёт
// в Telegram ФАКТ «было → стало». Под запрет авто-пересчётов (2026-06-05)
// это не попадает: пересборка снимка ничего не считает, она переносит в
// consultantBalance уже посчитанные суммы из commission и poolLog. Деньги
// по-прежнему считает только человек кнопкой.
//
// Закрытый месяц, дубли commission и расхождение выше порога автомат не
// трогает — зовёт человека (см. BalanceHealPolicy). Выключатель и порог —
// в админке: finance.balance_autoheal / finance.balance_autoheal_limit.
//
// Каждый час в рабочее время, а не раз в сутки: расхождение появляется
// после правок комиссий днём, и реестр не должен врать до утра. Сверка —
// один SELECT; тяжёлая пересборка запускается, только если дрейф есть.
Schedule::command('finance:autoheal-balances')
    ->weekdays()
    ->hourly()
    ->between('9:00', '19:00')
    ->name('balance-autoheal')
