  ...
  <v-layout>
    ...
    <v-main class="content-main" :class="{ 'content-main--full-bleed': isFullBleedRoute }">
      <v-container fluid :class="isFullBleedRoute ? 'pa-0' : 'pa-5 pa-md-8'">
        ...
        <!-- Логин без карточки партнёра: кабинет технически открыт, но все
             партнёрские данные недоступны (эндпоинты отвечают «Консультант не
             найден»). Раньше такой пользователь упирался в окно акцепта, где
             «Принять» падало с той же ошибкой и закрыть окно было нельзя. -->
        <v-alert
          v-if="showNoConsultantBanner"
          type="error" variant="tonal" density="compact"
          class="mb-3"
          icon="mdi-account-question-outline"
        >
          <div class="text-body-2">
            <strong>Аккаунт не привязан к карточке партнёра.</strong>
            Разделы с вашими данными (отчёты, клиенты, структура) работать не будут,
            пока поддержка не восстановит привязку.
          </div>
          <div class="text-caption mt-1">Напишите в поддержку через кнопку чата — укажите свой e-mail.</div>
        </v-alert>
...
// Логин без карточки партнёра (consultant.webUser). Для staff это норма —
// баннер только для тех, кто зашёл как партнёр.
const showNoConsultantBanner = computed(() =>
  !auth.isStaff && auth.user?.hasConsultant === false && !isFullBleedRoute.value);
...
const showOfferDialog = computed(() => {
  if (auth.isStaff) return false;
  // Нет карточки партнёра — принимать документы некому: /profile/accept-offer
  // отвечает «Консультант не найден», и окно намертво блокирует кабинет.
  // Вместо окна показываем баннер со ссылкой в поддержку (см. ниже).
  if (auth.user?.hasConsultant === false) return false;
...
const baseMenuItems = [
  ...
  { label: 'Дубли и связки', icon: 'mdi-vector-link', path: '/manage/duplicates', adminSection: 'partners', adminOnly: true },
