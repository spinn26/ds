  ...
  <div>
    ...
    <v-data-table-server :items="items" :items-length="total" :loading="loading"
      ...
      <template #item.actions="{ item }">
        ...
        <StartChatButton
          :consultant-id="item.partnerId || item.id"
          :consultant-name="item.partnerName || ''"
          context-type="requisites"
          :context-id="item.id"
          :context-label="`Реквизиты ${item.partnerName || ''}`"
        />
      </template>
      <template #no-data>
    ...
    <v-dialog v-model="rejectDialogOpen" max-width="480">
      <v-card class="pa-4">
        ...
        <v-card-text>
          <v-textarea v-model="rejectComment" label="Причина отклонения" rows="3" variant="outlined" />
        </v-card-text>
        <v-card-actions>
          <v-spacer />
