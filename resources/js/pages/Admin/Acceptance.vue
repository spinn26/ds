  ...
  <div>
    ...
    <v-card class="mb-3 pa-3">
      <div class="d-flex ga-2 flex-wrap align-center">
        ...
        <v-spacer />
        <ColumnVisibilityMenu :headers="headers" v-model:visible="columnVisible" storage-key="acceptance-cols" />
      </div>
    </v-card>
    ...
    <v-data-table-server :items="items" :items-length="total" :loading="loading"
      ...
      <template #item.chat="{ item }">
        <StartChatButton :partner-id="item.consultantId || item.consultant" :partner-name="item.consultantName || item.personName"
          context-type="Акцепт" :context-id="item.id" :context-label="item.personName || '#' + item.id" />
      </template>
