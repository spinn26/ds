<template>
  <div>
    <div class="d-flex align-center ga-2 mb-4">
      ...
      <h5 class="text-h5 font-weight-bold">Транзакции</h5>
    ...
    <v-data-table-server :items="items" :items-length="total" :loading="loading"
      ...
      density="compact" hover no-data-text="Транзакции не найдены">
      <template #item.amount="{ item }">
        {{ fmt(item.amount) }} {{ item.currencySymbol || '' }}
      </template>
      <template #item.type="{ value }">
        <v-chip size="x-small" :color="value === 'credit' ? 'success' : 'error'">{{ value }}</v-chip>
      </template>
    </v-data-table-server>
  </div>
</template>
