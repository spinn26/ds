<template>
  <div>
    <v-alert type="info" variant="tonal" class="mb-3" density="compact">
      ...
      Если поле пустое — используется значение из <code>.env</code>. Placeholder
      <code>Оставить без изменений</code> = значение уже сохранено.
    </v-alert>
    ...
    <div v-for="(items, group) in groupedItems" :key="group" class="mb-4">
      <v-card variant="outlined">
        <v-card-title class="d-flex align-center pa-3">
          ...
          <span class="text-body-1 font-weight-bold">{{ groupLabel(group) }}</span>
          <v-spacer />
  ...
  try {
    ...
    if (e.response?.status === 429) {
      ...
      showError(retry
        ...
        : 'Слишком много попыток за минуту.');
    } else {
