...
async function submit() {
  ...
  try {
    ...
    // 404 = у логина нет карточки партнёра. Техническое «Консультант не
    // найден» ничего не объясняет пользователю — он видит ошибку в окне,
    // которое нельзя закрыть, и не понимает, что делать.
    error.value = e.response?.status === 404
      ? 'Ваш аккаунт не привязан к карточке партнёра, поэтому принять документы нельзя. Напишите в поддержку — привязку восстановят.'
      : (e.response?.data?.message || 'Не удалось зафиксировать акцепт. Попробуйте позже.');
  } finally {
